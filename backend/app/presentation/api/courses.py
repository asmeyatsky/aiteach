...
def read_lessons_for_course(
    course_id: int, skip: int = 0, limit: int = Query(default=20, le=100, ge=1), course_repo: CourseRepositoryPort = Depends(get_course_repository)
):
...
def mark_lesson_complete(
    ...
    course_repo: CourseRepositoryPort = Depends(get_course_repository),
    user_progress_repo: UserProgressRepositoryPort = Depends(get_user_progress_repository),
):
